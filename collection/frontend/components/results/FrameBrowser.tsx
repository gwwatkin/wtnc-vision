/**
 * FrameBrowser.tsx — Frame-browser overlay for manual crossing creation.
 *
 ...
 * the onCreateCrossing callback — no direct api mutations from this component.
 *
 * @module components/results/FrameBrowser
 */

import { useState, useEffect, useRef, useCallback } from 'preact/hooks';
import type { FrameBrowserProps, FrameInfo, FramesMeta, CollectionConfig } from '../../types';
...
function getFrameConfig(): { spanS: number; limit: number } {
...
interface RiderBoxCanvasProps {
  imgEl: HTMLImageElement | null;
  riders: Array<{ box: number[]; status: string; number: string | null; raw_text: string | null; confidence: number }> | null;
}

function RiderBoxCanvas({ imgEl, riders }: RiderBoxCanvasProps) {
...
interface MainFrameViewProps {
  runLabel: string;
  frame: FrameInfo | null;
}

function MainFrameView({ runLabel, frame }: MainFrameViewProps) {
  ...
  return (
    <div class="frame-browser__main">
      {!f.processed
        ? <p class="frame-browser__no-outcome">No outcome data (frame not yet processed)</p>
        : null}
      <div class="frame-canvas-wrapper">
        ...
        />
        {f.processed
          ? <RiderBoxCanvas imgEl={imgEl} riders={riders} />
          : null}
      </div>
      {f.processed
        ? <div class="frame-browser__outcome"></div>
        : null}
    </div>
  );
}
...
interface AddCrossingRowProps {
  runLabel: string;
  frame: FrameInfo | null;
  onCreateCrossing: (payload: object) => Promise<void>;
}

function AddCrossingRow({ runLabel, frame, onCreateCrossing }: AddCrossingRowProps) {
  ...
  return (
    <div class="frame-browser__add-row">
      {!open
        ? <button type="button" onClick={handleOpen}>Add crossing here</button>
        : <div>
            <input
              type="text"
            ...
            {feedback ? <span class="frame-browser__feedback">{feedback}</span> : null}
          </div>
      }
    </div>
  );
...
export function FrameBrowser(props: FrameBrowserProps) {
  ...
  useEffect(() => {
    ...
    api.fetchFrames(runLabel, { anchorTs: windowAnchor, spanS, limit })
      .then((data) => {
        ...
        const d = data as { frames?: FrameInfo[]; meta?: FramesMeta };
  ...
  const handleScrub = useCallback((e: Event) => {
    ...
    const chosenMs = Number((e.target as HTMLInputElement).value);
  ...
  useEffect(() => {
    ...
    if (thumb) (thumb as Element).scrollIntoView({ block: 'nearest', inline: 'nearest' });
  }, [selectedIdx, frames]);
  ...
  return (
    <div class="frame-browser__overlay" role="dialog" aria-modal="true">
      ...
      {loading
        ? <p class="frame-browser__no-outcome">Loading frames…</p>
        : loadError
          ? <p class="frame-browser__no-outcome">{loadError}</p>
 ...
 */
function _formatTimeOfDay(isoOrDate: string | Date): string {
