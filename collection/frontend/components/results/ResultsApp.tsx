/**
 * ResultsApp.tsx — Root Preact component for the results page.
 *
 * Self-contained via COLLECTION_CONFIG; no props. Owns ALL results-page state
 * through a single `useReducer` (state.ts / FROZEN-4) and the poll loop. It is
 * the ONLY place server mutations + refresh live (FR13): child overlays call the
 * `on*` callbacks below, which hit `api.ts` then re-poll. The old
...
import type { ResultsAppProps } from '../../types';
import * as api from '../../api';
import { resultsFromCrossings, candidatesToResults } from '../../results/data';
import { reducer, initialState, hashPayload } from './state';
...
import { onBackendUrlChange } from '../../backend-url';

const _win = window as unknown as { COLLECTION_CONFIG?: { RESULTS_POLL_MS?: number } };
...
export function ResultsApp(_props: ResultsAppProps) {
  ...
  const loadResults = useCallback(async (label: string) => {
    if (!label) return;
    try {
  ...
  const loadStatus = useCallback(async (label: string) => {
    if (!label) return;
    try {
      const status = await api.fetchStatus(label);
      dispatch({ type: 'POLL_STATUS', status: status as object });
    } catch (_err) {
  ...
  useEffect(() => {
    ...
    async function tick() {
      ...
      loadStatus(run); // fire-and-forget
    }
    tick();
  ...
  const onEdit = useCallback(async (crossingId: string, fields: object) => {
    await api.postEdit(selectedRun || '', crossingId, fields);
    await refresh();
  }, [selectedRun, refresh]);

  const onDelete = useCallback(async (crossingId: string) => {
  ...
  const onPromote = useCallback(async (candidateId: string, payload: object) => {
    await api.promoteCandidate(selectedRun || '', candidateId, payload as { action: string; number: string });
    await refresh();
  }, [selectedRun, refresh]);

  const onDismiss = useCallback(async (candidateId: string) => {
    await api.dismissCandidate(selectedRun || '', candidateId);
    await refresh();
  }, [selectedRun, refresh]);

  const onCreateCrossing = useCallback(async (payload: object) => {
  ...
  const orderedCrossingIds = state.packs
    ...
    .map((r) => (r as import('../../types').Result).crossingId);
    ...
    <div class="results">
      <div class="results__toolbar">
        <RunSelector
          ...
          onChange={(runLabel: string) => dispatch({ type: 'SELECT_RUN', runLabel })}
        />
        ...
        <button
          ...
          onClick={() => dispatch({ type: 'OPEN_BROWSER', anchorTs: null as unknown as string })}
        ...
        <div class="results__download">
          <button
            ...
            onClick={() => downloadResults(state.selectedRun as string, 'csv')}
          >Download CSV</button>
          <button
            class="download-btn"
            disabled={!state.selectedRun}
            onClick={() => downloadResults(state.selectedRun as string, 'json')}
      ...
      <Timeline
        ...
        onSelect={(item: object) => dispatch({ type: 'OPEN_SIDEBAR', item })}
      />
      ...
      {state.sidebar.item && (
        <Sidebar
          ...
          onPromote={onPromote}
          onDismiss={onDismiss}
          onOpenBrowser={(anchorTs: string) => dispatch({ type: 'OPEN_BROWSER', anchorTs })}
        />
      )}
