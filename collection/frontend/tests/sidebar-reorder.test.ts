/**
 * sidebar-reorder.test.ts — Regression test for the sidebar's neighbour-based
 ...
 *
 * Globals are provided by happy-dom via Vitest's environment: 'happy-dom'.
 *
 ...
 */
function makeResult(crossingId: string): object {
  return {
    isCandidate: false,
    crossingId,
    raceNumber: 42,
 ...
 */
function mountSidebar(overrides: Partial<Parameters<typeof Sidebar>[0]> = {}): { container: HTMLDivElement; calls: Array<{ crossingId: string; neighbours: { earlierId: string | null; laterId: string | null } }> } {
  const container = document.createElement('div');
  document.body.appendChild(container);
  const calls: Array<{ crossingId: string; neighbours: { earlierId: string | null; laterId: string | null } }> = [];
  const props = {
    item: makeResult('b'),
    runLabel: '',
    ...
    onEdit: async () => {},
    onDelete: async () => {},
    onReorder: async (crossingId: string, neighbours: { earlierId: string | null; laterId: string | null }) => { calls.push({ crossingId, neighbours }); },
    onPromote: async () => {},
    onDismiss: async () => {},
    onOpenBrowser: () => {},
    ...overrides,
...
function btnByText(container: HTMLElement, text: string): HTMLButtonElement | undefined {
  return Array.from(container.querySelectorAll<HTMLButtonElement>('.sidebar__btn'))
