/**
 * backend-url.test.ts — Unit tests for backend-url.ts (FROZEN-1, page-split spec).
 *
 ...
 * backend-url.ts functions reference `document` dynamically (at call time)
...
// backend-url.ts references `document` dynamically at call time, so it picks
...
describe('cookie round-trip', () => {
  beforeEach(() => {
    ...
    (window as unknown as Record<string, unknown>)['COLLECTION_CONFIG'] = { BACKEND_URL: '' };
  });
...
describe('getBackendUrl fallback', () => {
  ...
  afterEach(() => {
    clearCookies();
    delete (window as unknown as Record<string, unknown>)['COLLECTION_CONFIG'];
  });
  ...
  it('returns COLLECTION_CONFIG.BACKEND_URL (normalized) when no cookie is set', () => {
    (window as unknown as Record<string, unknown>)['COLLECTION_CONFIG'] = { BACKEND_URL: 'http://fallback.local:8000/' };
  ...
  it('cookie takes precedence over COLLECTION_CONFIG fallback', () => {
    (window as unknown as Record<string, unknown>)['COLLECTION_CONFIG'] = { BACKEND_URL: 'http://config-default.local' };
