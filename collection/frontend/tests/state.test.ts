/**
 * state.test.ts — Reducer + deriveView transitions for ResultsApp (task8).
 *
 ...
 * transitions. Uses only state.ts + data.ts (both pure, importable in Wave B).
 */
...
import {
  ...
  deriveView,
  hashPayload,
} from '../components/results/state';
import { resultsFromCrossings, candidatesToResults } from '../results/data';
import type { Action } from '../types';

// --- fixtures --------------------------------------------------------------

function crossingPayload(o: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    crossings: [
      {
...
function candidatePayload(o: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    candidates: [
      {
...
function pollAction(rp: unknown, cp: unknown): Action {
  return {
    type: 'POLL_RESULTS',
...
describe('deriveView', () => {
  ...
  it('computes lanes for present categories', () => {
...
describe('POLL_RESULTS', () => {
  it('identical hash returns the SAME state object (no-op)', () => {
  ...
  it('changed payload recomputes packs/lanes and updates state', () => {
  ...
  it('candidates are excluded from packs by default (candidatesVisible false)', () => {
    const s1 = reducer(initialState, pollAction(crossingPayload(), candidatePayload()));
...
describe('TOGGLE_CANDIDATES re-derives from retained raw arrays', () => {
  it('toggles candidatesVisible and re-derives packs', () => {
    const s1 = reducer(initialState, pollAction(crossingPayload(), candidatePayload()));
...
describe('selection and overlay transitions', () => {
  ...
  it('OPEN_BROWSER / CLOSE_BROWSER toggle the browser overlay', () => {
  ...
  it('SELECT_ITEM sets selectedId without opening the sidebar', () => {
...
describe('runs and status transitions', () => {
  it('SET_RUNS stores the run list', () => {
    const s = reducer(initialState, { type: 'SET_RUNS', runs: ['r1', 'r2'] });
    assert.deepEqual(s.runs, ['r1', 'r2']);
  });

  it('SELECT_RUN resets derived state and dedupe key', () => {
