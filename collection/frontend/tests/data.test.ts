/**
 * data.test.ts — Pure-logic regression tests for results/data.ts.
 * Ported from data.test.js (task2, jsdoc-to-ts spec).
 * Assertions unchanged in substance (SC3/NFR6 regression net).
 */
...
import type { Result, CandidateResult } from '../types';
...
function makeCrossing(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    crossing_id: 'c1',
    number: '42',
...
function makeResult(overrides: Partial<Result & Record<string, unknown>> = {}): Result {
...
function makeCandidateResult(overrides: Partial<CandidateResult & Record<string, unknown>> = {}): CandidateResult {
  const time = overrides.time instanceof Date
    ? overrides.time
    : new Date('2024-06-15T10:00:00.000Z');
  return {
